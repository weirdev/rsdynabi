use crate::cpp_gen::CppHeaderGen;
...
impl CppHeaderGen for CppHeaderDecl {
    fn gen_cpp_header(&self) -> String {
        ...
        for class in &self.classes {
            header.push_str("\n");
            header.push_str(&class.gen_cpp_header());
            header.push_str("\n");
        }
        header
    }
    ...
    fn gen_cpp_header(&self) -> String {
        ...
        for constructor in &self.constructors {
            header.push_str(&format!(
                "    {}{}\n",
                self.name,
                constructor.gen_cpp_header()
            ));
        }
        for method in &self.methods {
            header.push_str(&format!("    {}\n", method.gen_cpp_header()));
        }
        header.push_str("};\n");
        header
    }
...
impl CppHeaderGen for CppVariableDecl {
    fn gen_cpp_header(&self) -> String {
        format!("{} {}", self.ty.gen_cpp_header(), self.name)
    }
}
...
impl CppHeaderGen for CppConstructorDeclOrDef {
    fn gen_cpp_header(&self) -> String {
        match self {
            CppConstructorDeclOrDef::Decl(decl) => decl.gen_cpp_header(),
            CppConstructorDeclOrDef::Def(def) => def.gen_cpp_header(),
        }
    }
}
...
impl CppHeaderGen for CppMethodDeclOrDef {
    fn gen_cpp_header(&self) -> String {
        match self {
            CppMethodDeclOrDef::Decl(decl) => decl.gen_cpp_header(),
            CppMethodDeclOrDef::Def(def) => def.gen_cpp_header(),
        }
    }
}
...
impl CppHeaderGen for CppMethodDecl {
    fn gen_cpp_header(&self) -> String {
        let mut header = String::new();
        header.push_str(&format!("{} {}(", self.ret_ty.gen_cpp_header(), self.name));
        for (i, arg) in self.args.iter().enumerate() {
            if i > 0 {
                header.push_str(",");
            }
...
impl CppHeaderGen for CppTypeSpecifier {
    fn gen_cpp_header(&self) -> String {
        ...
        header.push_str(&self.ref_spec.gen_cpp_header());
        header
    }
}
